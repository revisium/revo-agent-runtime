...
export const adjacentNodePackage = async (
  ...
  nodeName = process.platform === 'win32' ? 'node.exe' : 'node',
): Promise<AdjacentNodePackageFixture> => {
  const directory = await realpath(await mkdtemp(join(tmpdir(), 'revo-adjacent-node-package-')));
  ...
  const escapedDirectory =
    mutation === 'escaped_node'
      ? await realpath(await mkdtemp(join(tmpdir(), 'revo-adjacent-node-package-escaped-')))
      : undefined;
