...
import { createM1AgentDiscovery } from '../../../src/application/manager/index.js';
...
test('constructs synchronously, discovers exact agents, and never observes the port', () => {
  ...
  const discovery = createM1AgentDiscovery({ definitions: [buildAgentDefinition()] }, port);
...
test('rejects invalid definitions synchronously without observing the port', () => {
  ...
  expect(() =>
    createM1AgentDiscovery({ definitions: [buildAgentDefinition({ id: '' })] }, port),
...
test('rejects duplicate exact definitions synchronously without observing the port', () => {
  ...
  expect(() => createM1AgentDiscovery({ definitions: [definition, definition] }, port)).toThrow(
    AgentManagerError,
  );
  expectPortUnobserved(port);
});
