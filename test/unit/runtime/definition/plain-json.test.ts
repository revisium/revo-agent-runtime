...
test('exposes the exact frozen agent runtime limits', () => {
  expect(AGENT_RUNTIME_LIMITS).toEqual({
    ...
    schemaNodes: 8_192,
    resultNodes: 65_536,
    invocationMetadataBytes: 65_536,
    parameterBytes: 262_144,
    permissionBytes: 262_144,
    promptBytes: 4_194_304,
    workspacePathBytes: 16_384,
    outputPathBytes: 16_384,
    diagnosticCount: 16,
...
test('exposes the exact frozen agent fault messages', () => {
  expect(AGENT_FAULT_MESSAGES).toEqual({
    ...
    invocationInvalid: 'Agent invocation request is invalid.',
    invocationDuplicate: 'Agent invocation id is duplicated.',
    invocationUnknown: 'Agent invocation id is unknown.',
    workspaceInvalid: 'Agent workspace is invalid.',
    parametersInvalid: 'Agent parameters are invalid.',
    permissionsInvalid: 'Agent permissions are invalid.',
    resultSchemaInvalid: 'Agent result schema is invalid.',
    environmentInvalid: 'Agent environment is invalid.',
    outputPathInvalid: 'Agent output path is invalid.',
    outputConflict: 'Agent output path conflicts with an existing entry.',
    scratchFailed: 'Agent scratch preparation failed.',
    spawnFailed: 'Agent process failed to spawn.',
    authenticationFailed: 'Agent authentication failed.',
    permissionDenied: 'Agent permission was denied.',
    managerNotInitialized: 'Agent manager is not initialized.',
    managerClosed: 'Agent manager is closed.',
    shutdownFailed: 'Agent manager shutdown failed.',
    recoveryInvalid: 'Agent recovery input is invalid.',
