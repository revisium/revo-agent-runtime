...
test('positive xai preflight forwards the same ACP cwd, env, config, and command', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'revo-opencode-preflight-'));
...
test('header-only auth list blocks ACP spawn', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'revo-opencode-preflight-block-'));
